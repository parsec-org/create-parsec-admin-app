...
import useStyles, { captchaImage, formBottomSpacer, hiddenOkButton, logoImage } from './styles';
...
export default () => {
  ...
  const sendSmsAuthCode = useCallback(
    async (phone: string) => {
      ...
      try {
        // 优先使用 demo mock 接口，失败时回退到真实接口
        const res = await getSmsAuthCode({
          phone: values.phone,
          captchaCode: values.captchaCode,
          clientCode: data?.clientCode || '',
        }).catch(() => getSmsAuthCode({
          ...values,
          clientCode: data?.clientCode || '',
        }));
        if (res.code === 0) {
  ...
  const handleResetPassword = useCallback(async (formData: any) => {
    // 优先使用 demo mock 接口，失败时回退到真实接口
    postPublicForgetPwd({ ...formData }).catch(() => postPublicForgetPwd({ ...formData })).then(() => {
      Modal.success({
        icon: null,
        content: (
          <Result
  ...
  return (
    <div className={styles.forgotPasswordWarp}>
      <div className="header">
        ...
        <div className="subtitle">CIS 市场拓展平台管理系统-重置密码</div>
      </div>
      <ProForm
        className="form-warp"
        formRef={formRef}
      ...
      >
        ...
        />
        <div style={formBottomSpacer}>&nbsp;</div>
      </ProForm>
